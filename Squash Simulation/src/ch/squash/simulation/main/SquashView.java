...
public class SquashView extends GLSurfaceView implements SurfaceHolder.Callback {
	...
	public void hideHud() {
		mTxtHudBall.setVisibility(View.GONE);
		mTxtHudFps.setVisibility(View.GONE);

		mIsUpdateUi = false;
	}

	public void showHud() {
		mTxtHudBall.setVisibility(View.VISIBLE);
		mTxtHudFps.setVisibility(View.VISIBLE);
			...
			public void run() {
				while (mIsUpdateUi) {
					SquashActivity.getInstance().runOnUiThread(new Runnable() {
						...
						public void run() {
							// update *ps
							mTxtHudFps.setText(String.format("%.2f",
									SquashRenderer.getFps())
									+ " fps\n"
									+ String.format("%.2f",
											MovementEngine.getMps()) + " mps");
							// update ball
