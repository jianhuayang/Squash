...
final public class SettingViews {
		...
		public void onValueChange(final NumberPicker picker, final int oldVal,
			...
			case R.id.numTimeBack:
				if (!Settings.setKeyValue("backtime", SHORT_TIMES[newVal]))
					Log.e(TAG, "couldnt save new value for backtime, "
							+ SHORT_TIMES[newVal]);
				break;
			default:
				Log.w(TAG,
	...
	private SettingViews() {
		// find views
		numSeries = (NumberPicker) MainActivity.getActivity().findViewById(
				R.id.numSeries);
		numBreak = (NumberPicker) MainActivity.getActivity().findViewById(
				R.id.numBreak);
		numCornerTime = (NumberPicker) MainActivity.getActivity().findViewById(
				R.id.numCornerTime);
		numCorners = (NumberPicker) MainActivity.getActivity().findViewById(
				R.id.numCorners);
		numMinTime = (NumberPicker) MainActivity.getActivity().findViewById(
				R.id.numMinTime);
		numMaxTime = (NumberPicker) MainActivity.getActivity().findViewById(
				R.id.numMaxTime);
		numTimeFront = (NumberPicker) MainActivity.getActivity().findViewById(
				R.id.numTimeFront);
		numTimeBack = (NumberPicker) MainActivity.getActivity().findViewById(
				R.id.numTimeBack);

		radCorners = (RadioButton) MainActivity.getActivity().findViewById(
				R.id.radCorners);
		radTime = (RadioButton) MainActivity.getActivity().findViewById(
				R.id.radTime);

		radioGroup = ((RadioGroup) MainActivity.getActivity().findViewById(
				R.id.radioGroup1));

		// calculate some final values
		LONG_TIMES = new String[4 * 9 + 1];
		for (int i = 0; i < LONG_TIMES.length; i++)
			LONG_TIMES[i] = Float.toString(1f + 0.25f * i);
	...
	private void initializeViews() {
		...
		// radiogroup: bool
		final boolean cornerOrTime = Settings.isCornersOrTime();
		radioGroup.setOnCheckedChangeListener(checkedChangeListener);
		radCorners.setChecked(cornerOrTime);
		radTime.setChecked(!cornerOrTime);
		checkedChangeListener.onCheckedChanged(radioGroup,
				cornerOrTime ? radCorners.getId() : radTime.getId());

		// corners: CORNERS
		numCorners.setMaxValue(CORNERS.length - 1);
		numCorners.setMinValue(0);
		numCorners.setDisplayedValues(CORNERS);
		// numCorners.setValue(Arrays.binarySearch(CORNERS,
		// Settings.getCorners()));
		...
		numMinTime.setValue(Arrays.binarySearch(LONG_TIMES,
				Settings.getMinTime()));
		numMinTime
				.setDescendantFocusability(NumberPicker.FOCUS_BLOCK_DESCENDANTS);
		...
		numMaxTime.setValue(Arrays.binarySearch(LONG_TIMES,
				Settings.getMaxTime()));
		numMaxTime
				.setDescendantFocusability(NumberPicker.FOCUS_BLOCK_DESCENDANTS);
